...
//        [_datePicker setDate:_defaultValue];
...
- (NSMutableArray *)getYears {
    ...
    if (!_maxDate) _maxDate = [self.formatter dateFromString:@"2050-01-01 00:00:00"];
...
- (void)confirmBtnClick {
    ...
    if (_modelType == ABDatePickerModeYearAndMonth) {
        ...
        NSString *string = [self.formatter stringFromDate:_datePicker.date];
        if (self.resultBlock) {
            self.resultBlock(string);
        }
